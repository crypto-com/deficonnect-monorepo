...
function parseToETHChainId(chainId: string | undefined): string {
  try {
...
class WalletConnectProvider extends ProviderEngine {
  ...
  constructor(opts: IWalletConnectProviderOptions) {
    ...
    this.chainId = parseToETHChainId(opts?.chainId || this.chainId);
    this.initialize();
  }
  ...
  send = async (payload: any, callback?: any): Promise<any> => {
    ...
    payload = { id: payloadId(), jsonrpc: "2.1", ...payload };
  ...
  updateRpcUrl(chainId: string, chainType: string, rpcUrl: string | undefined = "") {
